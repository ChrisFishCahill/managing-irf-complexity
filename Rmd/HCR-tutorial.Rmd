...
We then read in the fits that we want to pass to the harvest control rule simulator.  In this case, we choose the Beverton-Holt fits where the informative prior for compensation ratio was set at 6, read them in, and set up a vector of lake names (separated by underscores like we needed at the beginning of our get_hcr() function):
...
This would run 144 different simulation experiments given the Beverton-Holt compensation ratio prior = 6 BERTA fit.
