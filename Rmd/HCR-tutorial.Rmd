...
You need to be careful here--these $w_t$ values in point 4 are now different than the originally estimated values from our Bayesian models.  
...
Recall that t = 1 corresponds to year 1990 in our estimation model.  Again, this gave us recruitment anomaly sequences that looked qualitatively similar to the values we estimated (we will plot some of them below to show you in a bit). 

## Setting up the recruitment anomaly sequences (for real this time)

This time, in code....

We first extract the estimated $w_t$ values from the posterior and set up $w_{t_{historical}}$: 
...
Once we correct the `wt_historical` sequence by subtracting `wt_bar`, we just repeat the sequence `n_repeats` number of times.  This preserves the historical frequency and relative strength of strong and weak year classes that we estimated in the assessment model. Note that we are limited to the historical 26 year reference period for our management strategy simulations.  In all our simulations we set this at 8 repeats, and so simulations are 8 x 26 = 208 years in length. A simple rule of thumb here is to run the simulation at least ten times the generation length of the fish you are working on, which for Walleye around 200+ years. 
...
If you think about it, we cannot "see" any longer term patterns or event occurrences than have been visible during the historical reference period.  In this regard, we are to some extent beholden to our historical experiences with a given stock or fishery.  

Up to now we have specified the $w_t$ sequences identically to how we did for the shrimp problem, but here is where things change. 
