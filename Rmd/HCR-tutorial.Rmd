...
I am using the tidybayes package to manage Bayesian model output.  I'm sure there are other ways to do this, but I found this way to be the most efficient and effective for my purposes, and so now you have to learn it. Let's assume for now that `n_draws = 1`.  The next chunk of code is: 
...
So this piece of code takes a specific model fit, pipes it into the function `spread_draws()` to access this specific model fit's posterior, which is then passed to `sample_draws()` which randomly samples some number of draws from the posterior (i.e., rows of the posterior where columns are parameter names and rows are parameter values).  Note also that we are specifying only to grab three parameters--$R_0$, $ln(ar)$, and $br$.  These values are stored in the object `devs`. 

We then run one more line of code that figures out which specific draw(s) of the posterior we randomly selected, and stores this value as `draw_idx`.  This is important when we sample more than one draw because we want our samples to come from the same (randomly selected) rows of the posterior, as this preserves covariation among the parameter estimates used in our simulation (cool). 
...
You'll note we are also subsetting our Bayesian posterior model fits. Instead of using the full 1980-2018 numbers at age matrix and $w_t$ values, we'll subset these estimates to correspond to years 1990-2015.  We do this for two reasons.
...
As our main goal was to design harvest control rules robust to the recruitment variation present in these systems, we felt it was best to use this 1990-2015 range to initialize our age-structured simulation below.
