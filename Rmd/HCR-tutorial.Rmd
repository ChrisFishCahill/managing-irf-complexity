...
my_lines %>%
  ...
  ylab("Total allowable catch (kg/ha)") + 
  xlab("Biomass of finned critters vulnerable to fishing (kg/ha)") + 
  ggtitle("Thinking about a few possible linear policies")  
```
...
my_grid %>%
  ...
  ggtitle("The `space` of all possible simple linear policies")
...
The key thing to recognize is that each possible combination of $b_{lrp}$ and $c_{slope}$ defines a single harvest policy.  We want to test the performance of each of these policies, so we need to figure out a range of policies to iterate across for each lake.
