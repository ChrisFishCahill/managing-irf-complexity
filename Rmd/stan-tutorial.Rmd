...
``` 
In Stan, characters occurring after a `//` are not run.  Thus, these are similar to using `#` in R and can be used to comment code.
...
## Problem statement 
Let's suppose that some poor soul's dissertation counted Gorbachev's Homing Mussels at various locations along a 30 km river.  The experimental design of this survey started in the headwaters (km 0) and proceeded downstream (km 30), sampling mussels via a point count surveys every 5 km. Surveys were repeated at each location five times (misery).

Your biologist friends tell you there is some industrial development planned for km 18.7, and they need to determine what the expected density of critters might be in that location given these data so that they can evaluate different management options. However, point count surveys only occurred at kms 15 and 20.  Can you help them? 
...
Note that for the following code to work, you will have to save a .stan file (possible in R studio).  I named my script `mussels.stan`. 
...
Voila, you've now coded a Bayesian Generalized Linear Model in Stan. The term y_pred is giving you the posterior predictive distribution (see Gelman et al. 2013) for the number of Gorbachev's Homing Mussels that are likely present at river km 18.7, complete with uncertainty intervals. 
...
Stan, in the background, then creates a log probability function as the log of the priors plus the log likelihood(s). This log probability function is repeatedly evaluated for different random draws of the priors and passed to an algorithm that uses magic to accept some proportion of the random draws and which also helps it walk toward the most likely parameter values.  

Lastly, Stan will then pass these accepted parameter values to the generated quantities block.

Stan does all this for you, which is pretty dope. 
...
1. Can you adapt the mussels.stan code to address this criticism? 
2. How does your model compare to the Frequentist fit using the `glm()` function? 
3. Do your answers change if you change the prior sd terms? Note you will have to compile the .stan model each time you change it. 
