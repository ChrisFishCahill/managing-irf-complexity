...
`purrr` and `furrr` are functional programming tools for the R programming environment. A great resource on these tools is available at [https://homerhanumat.github.io/r-notes/purrr-higher-order-functions-for-iteration.html](https://homerhanumat.github.io/r-notes/purrr-higher-order-functions-for-iteration.html)
...
Thus, the 'walk' bit just says that this function is the silent analog of 'map'.  `pwalk()` is running exactly the same thing as `pmap()`, but it isn't storing the results anywhere or kicking them out to the console.  Why might this be useful? 
