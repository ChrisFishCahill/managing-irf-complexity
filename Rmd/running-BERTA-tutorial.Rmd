...
---
# Goals
- Import both the FWIN and stocking data, and show how one might add new data to the data used in Cahill et al. 2021
...
First, `year` in these files are indexed from year 2000, which was when the first FWIN survey in Alberta occurred.  Thus, year = 1 is 2000, year = 2 is 2001, etc.  
...
- calculate the proportion of fish aged in that survey in the `p_aged` slot
- put the catch at each age from 0 to 25 in columns labelled '1', '2', ..., '25'
- all other columns just need to be copied for the lake of interest

To add new information to the stocking dataframe, note that rows are lakes and columns are the number of Walleye stocked per hectare during 1980-2018. Thus, if you want to add stocking values after 2018 you'd add columns for the lake(s) of interest.  These are indexed from 1980 because BERTA hindcasts to reconstruct recruitment trends in years pre-FWIN.
...
Note how this will break if you start using informative priors other than 6 or 12.  How could you fix this issue?

Now that we've done all this work, it is dead simple to fit a single BERTA model to any lake in the FWIN database: 

```{r, eval = FALSE}
# do a short run with one chain 
...
That works fine, now let's do this exact same thing again using a simple `map()` function from {purrr}.  Why we do this will make more sense in a bit.
...
Once we have this set up, we call `plan()`, `get_fit()`, and `future_pwalk()`: 
...
For all the benefits of functional programming, it is certainly harder to debug code when errors inevitably arise.  This means you should have a working knowledge of `browser()` and a few other Rstudio tricks. **Discuss this in person**.
