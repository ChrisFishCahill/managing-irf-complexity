...
# Okay, but why all this jank {purrr} stuff? 
...
```
Okay, so now we are just playing dirty.  Using {furrr} we just ran the same calculation on a laptop in roughly 1/8 of the amount of time it takes to do that same calculation using a base R loop. Obviously this isn't entirely an apples to apples comparison, but the point stands that this is fast and powerful.
...
# create a tibble to iterate through, where columns correspond to function arguments in get_fit()
...
```
Boom, we just ran 6 lakes x 2 stock-recruitment x 2 compensation ratios = 24 Bayesian population dynamics models in only a few lines of code. We saved each of these model runs in the fits folder. How could you add another lake? Or an entirely different set of lakes? 
